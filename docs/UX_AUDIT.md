...
**High impact**
1. ◐ Shared week/season context + one default-week rule + parameter-carrying cross-links (Home → weekly pages; Game ↔ Preview ↔ Matchup Bets; Team pages ↔ each other). — Value Bets/Matchup Bets/Prop Bets trio done; no app-wide context, no shared default-week rule, **Home has no "this week" strip at all**, Team Comparison has zero outbound links.
...
3. ◐ Conclusion-first restructuring of the decision pages: model disagreement visibility (Week Preview), accuracy-by-confidence answer (Model Overview), prioritized player pivots (Value/Matchup Bets), picks summary (Game Picks). — Model Overview, Value/Matchup Bets pivots, and Game Picks record chip are done; Week Preview model-disagreement dot-strip is done; the two "who wins this week" *engines* (Spread Win % vs Matchup Previews) still don't cross-reference each other.
4. ◐ Trust fixes: dead turnover rows, Scorecards labeling, engine-disagreement transparency between the two pick systems. — Turnover rows and Scorecards labeling fixed; **engine-disagreement transparency between Spread Win % and Matchup Previews is NOT done.**
5. ◐ Literacy layer: glossary + one-line method captions for statistical/betting jargon, win-type legend. — Captions/tooltips done per-page; **no single shared/unified glossary** (two separate glossary implementations remain).

**Medium impact**
6. ✅ Win Types: comparative trend view above the per-season blocks. — Done (KPI trend lines + drill-down blocks, session 2026-07-17).
7. ◐ Grade context everywhere grades appear (rank/percentile chips sourced from the Weekly-tab statistics). — Done on Team Comparison; not confirmed on Matchup tab / Scorecards hero card grade display beyond league rank already logged for Scorecards.
...
9. ◐ Color-independent encodings for winners/picks/win types. — Done for Game Picks winner marking; **Win Types and Spread Win % remain color-only for win-type category** (Spread Win % code comment explicitly flags and defers this).
...
12. ☐ Pick/parlay persistence consistency; donut de-emphasis where a single number carries the message. — **Parlay Builder still does not persist (no localStorage), unlike Game Picks.** Donut de-emphasis not confirmed as a general pass.
...
- Parlay Builder: localStorage persistence — **decided against**: user wants the builder to *reset* (clear-all button added), not persist across visits. Season-type-ignoring player list — still open, not revisited. (Inert Week control — **removed entirely** 2026-07-20, no longer applicable.)
- Scorecards / Matchup tab grade-rank context beyond what's already logged — worth a quick audit-vs-code pass next session.
