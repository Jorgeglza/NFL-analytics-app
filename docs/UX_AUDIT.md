...
> **Status (2026-07-20):** Per-page audit items §1–13 are implemented (see `IMPLEMENTATION_LOG.md` M3/M4 session notes, page by page). The **Cross-Page Review / Global Opportunities / Prioritized Summary below were not tracked per-item until now** — status legend added inline (✅ done · ◐ partial · ☐ not started). Full breakdown in `IMPLEMENTATION_LOG.md` under "M4 cross-page/general audit items".

# Cross-Page Review

## Repeated components that should share one pattern
1. ✅ **Stat selectors** — five player pages + Grading Model Teams tab all expose the raw pipeline stat list, in three formats (snake_case ~44 items, snake_case ~130 items, Title Case ~130 items). One shared, curated, grouped stat picker (common prop stats first, advanced tier below, consistent display names) is the single highest-leverage shared fix. — *Done: `pages/player-analysis/statPicker.ts` shared across Prop Bets, Parlay, Player Team Stats, Matchup Bets, Value Bets, Grading Model Teams tab.*
2. ✅ **Player pivot table** (Prop Bets, Matchup Bets, Value Bets) — same weekly-columns pivot in three places with different row-inclusion and highlighting rules. Good candidate for one component with one relevance-ordering and one "missing week = bye" convention, explained once. — *Done: shared pivot pattern + rankInTeam prioritization + opponent/bye footnote convention, applied across the three pages.*
3. ✅ **Set Line → hit-rate detail** (Prop Bets, Matchup Bets, Parlay legs) — same interaction three times; should look and phrase results identically (including "N of M, X%" wording and implied-odds framing if adopted). — *Done: "N of M, X%" + implied-fair-odds verdict sentence shared across Prop Bets/Parlay/Matchup Bets legs.*
4. ◐ **Grades display** (Team Comparison, Matchup tab, Grading Model) — grade numbers appear with different framing everywhere and with distribution context only on Weekly tab. One shared "grade chip" convention (value + rank/percentile) would propagate context app-wide. — *Partial: Team Comparison grades now show league rank (session 2026-07-17); no shared "grade chip" component — each page still builds its own grade+rank display.*
5. ◐ **Win-type vocabulary** — full labels (Game Picks, Win Types, Spread Win %), FH/FA/UH/UA codes (Week Preview), colors implied by charts. One legend component + consistent color mapping. — *Partial: legends/glossaries added per-page (Game Picks color legend, Win Types glossary panel, Week Preview FH/FA decoder) but each is a separate implementation, not one shared component.*
6. ✅ **×N collision markers, low-N greying, "Low N" notes** — good honesty patterns that already recur; codify them so every small-sample number in the app gets the same treatment. — *Present consistently: Spread Win % bucket table (greyed low-N), scatter collision markers, Weekly Picks low-N notes.*

## Overlapping objectives
- ☐ **Three "who wins this week" surfaces**: Spread Win % Weekly Picks (bucket-historical engine), Matchup Previews Week Preview (4-model engine), Game Picks (manual). They can *disagree* — e.g. 2025 Week 18 CAR@TB: Spread Win % recommends **Underdog away (CAR)** while Matchup Previews picks **TB** (which won). Nothing tells the user these are different engines or how each has performed. Either present them as competing models in one place or clearly position each page's engine and cross-reference. — **NOT DONE: no link/reference between Spread Win % and Matchup Previews anywhere in the code.**
- ✅ **Matchup Bets vs Value Bets**: same mismatch idea at two scopes with different rank constructions and different score scales, unexplained. Position as a two-step journey (Value Bets = weekly radar → Matchup Bets = game drill-down). — *Done (2026-07-19): Matchup Bets merged into Value Bets as a drill-down, callout explains the methodology difference, zoom-in links carry season/week/game/stat/player.*
- ☐ **Scorecards vs Team Comparison vs Grading Model Teams**: three team-centric views (identity / head-to-head / model drivers) that never link to each other despite sharing the team+season selection. — **NOT DONE: Team Comparison still has zero outbound links (verified in code — no `Link`/`useNavigate` in `TeamComparison.tsx`); log explicitly notes this was "Deferred."**

## Disconnected user journeys / gaps
- ◐ **No context carry-over**: every page independently defaults and re-asks season/week/team. Following one game from Game Picks → Matchup Preview → Matchup Bets means re-selecting the same game three times. A shared season/week (and where sensible, team/game) context — or at minimum links that carry parameters — is the biggest navigation win available. — *Partial: `useSearchParams` param-carrying links exist for the Value Bets ↔ Matchup Bets ↔ Prop Bets trio and Grading Model Weekly→Teams. No app-wide shared context; Home, Team Comparison, Scorecards, Win Types, Spread Win %, Matchup Previews are not wired into it.*
- ☐ **Inconsistent week defaults**: Game Picks lands on Week 22 (last row in data), most pages on Week 18, Matchup Bets derives "current week" from browser timezone. One shared "default week" rule. — **PARTIALLY addressed only for Game Picks** (now defaults to current in-progress/last-completed week, session 2026-07-17); Matchup Bets browser-tz deviation is still explicitly documented as unresolved (log: "Deviation: browser-local tz for default week (not America/Monterrey)"). No single shared rule/helper exists.
- ☐ **No onward links anywhere**: zero cross-page links exist outside the navbar. — *Now false in general (several cross-links added — see Matchup Bets/Value Bets/Prop Bets above), but still true for Home, Team Comparison, Scorecards, Win Types, Spread Win %, and the two "who wins this week" engines relative to each other.*

## Global opportunities
- ◐ 🔴 **Terminology & literacy layer**: the app assumes fluency in Wilson CIs, lift, overround, EPA, WOPR, rank inversions, and its own win-type taxonomy. A consistent lightweight explanation pattern (one-line captions/tooltips, one glossary — which the Features tab already promises but doesn't deliver) would widen the audience without dumbing anything down. — *Partial: per-page tooltips/captions added widely (Spread Win % control tooltips, Grading Model model-explainer card, Win Types glossary). **No single shared glossary** — Win Types glossary and Grading Model Features-tab glossary are two independent implementations (one an in-page panel, one an external link).*
- ◐ 🔴 **Conclusion-first framing**: several strong analytical blocks (Spread Pick Engine, Trend Edge, Model Overview, Value Bets pivot) present evidence before, or instead of, the verdict. The app's data is decision-grade; its pages should lead with the decision. — *Done for Spread Win % (verdict card), Matchup Previews (verdict strip, model breakdown), Value Bets (prioritized pivot), Model Overview (confidence-band summary leads, per-cell % moved to hover). Not revisited: Trend Edge block inside Matchup tab still asks the user to synthesize which team it favors (per original audit note) — not confirmed fixed.*
- ✅ 🟡 **Render weight**: Win Types (~22 charts), Player Team Stats (32 charts), Model Overview (2,300-cell grid) are heavy enough to time out headless screenshots; lazy/on-demand rendering would help real users on modest hardware too. — *Done: `components/LazyMount.tsx` shared IntersectionObserver component applied to Win Types (drill-down + LazyMount) and Player Team Stats (32 cards); Model Overview keeps the grid but leads with a confidence-band summary and moved per-cell numbers to hover-only.*
- ◐ 🟡 **Accessibility**: winner/pick correctness and win types are frequently color-only encodings; small uppercase micro-labels run ~10px; canvas charts have no text alternative. Adding non-color redundancy (marks, text) for the key encodings is the priority subset. — *Partial: Game Picks winner now bold+✓ (non-color). **Win Types stacked bars and Spread Win % remain color-only for win-type category** (verified in code — `SpreadWinPct.tsx` comment explicitly acknowledges the color-only encoding rather than fixing it).*
- ✅ 🟡 **Responsiveness**: multi-column layouts (Team Comparison 3-col, wide pivots with 18 week columns) need a defined small-screen behavior; wide tables should scroll within their cards. — *Done: `overflow-x-auto` convention applied consistently — `components/ui.tsx`, Team Comparison, Game Picks, Spread Win %, Grading Model tabs, Model Overview pick matrix.*
- ◐ 🟢 **Empty/edge states**: sparse-week views (Week 22 = 1 game), pre-selection states (Parlay), and null-data rows (turnovers) each currently render "less page" rather than explaining themselves. — *Partial: turnover nulls fixed at pipeline source (no longer applicable); Grading Model bye weeks now show an explicit "Bye" cell. Parlay Builder's empty/pre-selection state guidance not confirmed addressed.*

## Data inconsistencies noted (source-level, not presentation)
1. ✅ Turnover margin / turnovers / turnovers-allowed are null in pipeline data → dead rows in Team Comparison and a permanently zero "turnover margin" driver row in Grading Model Teams tab. (Known issue; resolving it upgrades two pages at once.) — *Fixed at pipeline source (2026-07-17): `_normalize_weekly()` renames nflreadpy's `passing_interceptions` back to `interceptions`; 2025 turnovers now fully populated. Note: this changed 2025 grades (documented consequence in the log).*
2. ✅ Scorecards stat-card value↔label pairing reads wrong (totals under per-game labels) — verify whether labels or adjacency are at fault. — *Fixed: Scorecards full rework gives every stat row explicit per-game/total(N gm)/league-avg labels.*
3. ☐ "All-Time Matchup" label vs 2015+ data coverage. — **NOT explicitly confirmed fixed** — not mentioned in any session note; verify the Matchup tab's "All-Time Matchup"/"Head-to-Head" section still needs the "since 2015" rename (a "Head-to-Head (since 2015)" rename IS logged for session 2026-07-17, so this is likely done — confirm on next Matchup tab visit).
4. ◐ Parlay Builder's inert Week control and season-type-ignoring player list (preserved quirks — decide, don't inherit). — *Not resolved as a product decision; quirks explicitly still preserved per log ("Quirks kept: Week dropdown unused in calc; player list ignores season_type").* **Also still open: Parlay Builder does not persist across visits** (verified in code — no `localStorage` usage in `ParlayBuilder.tsx`, unlike Game Picks' manual winners).

---

# Prioritized Summary (application-wide)

> Status column added 2026-07-20 after a code-level cross-check (not just log claims). ✅ done · ◐ partial · ☐ not started.

> **Reconciled 2026-07-21** against current source (not just prior log claims) — see `IMPLEMENTATION_LOG.md`'s "Next" note dated 2026-07-21. Items 1, 4, 7 below were previously flagged as open based on stale audit notes; `Scorecards.tsx` and `MatchupTab.tsx` were confirmed in source to already show grade + league rank, and Home/Team Comparison/glossary were confirmed done in earlier 2026-07-20 sessions.

**High impact**
1. ✅ Shared week/season context + one default-week rule + parameter-carrying cross-links (Home → weekly pages; Game ↔ Preview ↔ Matchup Bets; Team pages ↔ each other). — Home launchpad, `currentWeek()` shared default-week rule (Home/Game Picks/Team Comparison), and cross-links (Team Comparison → Matchup Preview/Scorecards, Game Picks → Matchup Preview/Team Comparison) all done. Matchup Bets intentionally keeps its own `defaultWeekNearToday()` helper (answers a different question — closest week to today vs. current-season in-progress week) rather than merging into `currentWeek()`.
2. ✅ One curated, grouped, consistently-named stat picker across all player pages and the Teams tab. — `statPicker.ts`, fully applied.
3. ◐ Conclusion-first restructuring of the decision pages: model disagreement visibility (Week Preview), accuracy-by-confidence answer (Model Overview), prioritized player pivots (Value/Matchup Bets), picks summary (Game Picks). — Model Overview, Value/Matchup Bets pivots, Game Picks record chip, and Week Preview model-disagreement dot-strip are all done; the two "who wins this week" *engines* (Spread Win % vs Matchup Previews) still don't cross-reference each other — **explicitly declined by the user (2026-07-20)**, not an open gap.
4. ✅ Trust fixes: dead turnover rows, Scorecards labeling, engine-disagreement transparency between the two pick systems. — Turnover rows and Scorecards labeling fixed; engine-disagreement transparency **explicitly declined by the user (2026-07-20)** rather than left undone by oversight.
5. ✅ Literacy layer: glossary + one-line method captions for statistical/betting jargon, win-type legend. — `lib/glossary.ts` + `components/Glossary.tsx` is the single shared glossary, used by both Win Types and Grading Model's Features tab (done 2026-07-20).

**Medium impact**
6. ✅ Win Types: comparative trend view above the per-season blocks. — Done (KPI trend lines + drill-down blocks, session 2026-07-17).
7. ✅ Grade context everywhere grades appear (rank/percentile chips sourced from the Weekly-tab statistics). — Confirmed in source: Team Comparison, Scorecards (`Scorecards.tsx`'s `gradeInfo` shows `#{rank}` per grade), and Matchup tab (`MatchupTab.tsx`'s `gradesIdx.rank`) all show grade + league rank.
8. ✅ Render-on-demand for the three heaviest pages; defined responsive behavior for wide tables/multi-column layouts. — LazyMount on Win Types + Player Team Stats; `overflow-x-auto` convention applied broadly.
9. ◐ Color-independent encodings for winners/picks/win types. — Done for Game Picks winner marking; Win Types/Spread Win % color-only win-type encoding **explicitly declined by the user (2026-07-20)**, not an unaddressed gap.

**Low impact**
10. ◐ Empty/edge-state explanations (byes, sparse weeks, pre-selection guidance). — Grading Model bye cells done; Parlay Builder pre-selection guidance not confirmed.
11. ✅ League-leaders strip on Player Team Stats; jump navigation on long pages. — Done (league-wide leaders strip + conference/division jump chips, session 2026-07-18).
12. ✅ Pick/parlay persistence consistency; donut de-emphasis where a single number carries the message. — Parlay Builder intentionally **resets** rather than persists (user decision 2026-07-20, Reset button added) — a deliberate product choice, not an inconsistency. Donut de-emphasis not confirmed as a general pass (low-impact, not tracked further here).

## What's still missing (updated 2026-07-20, Session 7 cont.×2)
- ✅ Home page "this week" launchpad — **done**: current week/games/date range + a link into Game Picks with season/week carried.
- ✅ Single shared glossary component — **done**: `lib/glossary.ts` + `components/Glossary.tsx`, used by both Win Types and Grading Model Features tab.
- Cross-reference / disagreement callout between Spread Win % Weekly Picks and Matchup Previews Week Preview — **declined by user (2026-07-20)**: user asked to investigate a suspected ties-handling bug instead of adding a callout; that bug was found and fixed (see log). No cross-page callout exists; not planned unless requested again.
- ✅ Team Comparison → Matchup Preview / Scorecards cross-links — **done**.
- ◐ App-wide shared week/season context — **partially done**: one shared `currentWeek()` default-week rule now used by Home/Game Picks/Team Comparison (user explicitly scoped this to "shared default-week logic only," not full cross-page state sync). Matchup Bets computes its own default week from "closest gameday to now" (confirmed correct — timezone-agnostic `Date.now()`, not actually region-dependent despite the old comment; deduped 2026-07-20 to import the shared `defaultWeekNearToday()` instead of a local copy) — it answers a different question (closest week to today, any season) than `currentWeek()` (current-season in-progress/last-completed week), so left as its own helper rather than merged into `currentWeek()`.
- ☐ Win Types + Spread Win % win-type color-only encoding (no text/pattern redundancy) — **explained twice, explicitly declined for now (2026-07-20)**: user asked what it meant, got the explanation, declined to implement. Revisit only if asked again.
- ✅ **Ties get their own category** (new, user-requested 2026-07-20 — not from the original audit): Win Types' `Category` gained an 8th value `"Tie"`, split out of the old ambiguous "(No Score)" bucket (which conflated ties with unplayed games). Both Win Types and Spread Win % now exclude ties entirely from every win-rate percentage (previously ties silently counted as a favorite loss). Verified against raw schedule data.
- Parlay Builder: localStorage persistence — **decided against**: user wants the builder to *reset* (clear-all button added), not persist across visits. Season-type-ignoring player list — **confirmed done in source (2026-07-21)**: `ParlayBuilder.tsx`'s `typed` (and everything derived from it — teams, stats, players, calc) filters by `leg.seasonType`; the "ignores season_type" note was stale, left over from an earlier build. (Inert Week control — **removed entirely** 2026-07-20, no longer applicable.)
- Scorecards / Matchup tab grade-rank context — **confirmed done in source (2026-07-21)**: both already show grade + league rank (`Scorecards.tsx`'s `gradeInfo`, `MatchupTab.tsx`'s `gradesIdx.rank`). No audit-vs-code gap remained.
- Prop Bets / Parlay Builder: full team names in dropdowns, random starting team+stat, top-player default, Parlay Week control removal — all **done** 2026-07-20 (direct user request, not from the original audit).
