...
- ✅ /game_analysis/spread_win_percentage (now **Spread Analytics**, 2 tabs) — Win Rate & Calibration tab: filters (multi season/week, win types, bin size, signed/abs, min-N, CI), 6 KPIs, calibration/stacked/heatmap/lift charts, bucket table, Weekly Picks panel. KPIs + bin aggregates + Wilson p̂ verified exact vs pandas replica. Grid-aligned buckets replace pd.cut edges (deviation: pandas silently dropped a game whose |spread| hit the exact top edge; we keep it). 2026-07-22: added a compact "Recommended vs Actual vs Historic" chart inside Weekly Picks (between the KPI row and the details table) — per-week pick logic factored into `computeWeekPicks()` (shared with the single-week table). Iterated same day: an initial version averaged Recommended/Actual across every graded week up to the selection (192 weeks by Week 18), which barely moved between weeks — replaced with per-selected-week Recommended (from `reco.chips`) vs that week's own graded Actual outcomes vs the long-run Historic baseline, so the chart now visibly reflects each week's own spreads. Also removed a redundant per-week count chart (chips already showed it) and labeled the chips "Recommended distribution." No pipeline/data changes; computed entirely client-side from `schedule.json`. 2026-08-03: renamed to Spread Analytics, moved to `spread-analytics/WinRateTab.tsx` (near-verbatim) with a new Weekly Breakdown tab alongside it — see that session's log entry. 2026-09-19: Pick'em → "This Week" sub-view's per-team momentum table (`pickem/ThisWeekView.tsx`) reworked for clarity per user feedback — it wasn't obvious the table was a pooled-across-seasons historical split rather than a live streak. Reworded heading/added an explainer paragraph spelling out the mechanic in plain language, and added `InfoDot` tooltips to each column header. Team names are now clickable, opening a new `TeamMomentumDetail.tsx` modal (reuses `components/Modal.tsx`, styled like `TeamWeekModal.tsx`) showing: the headline KPIs for that team, a season-by-season table of the exact Week(W-1)→Week(W) result pairs pooled into the row (so a user can see precisely which seasons drove the swing), and a game-by-game log (opponent, score, result, win type via `winType()`) for the most recent season on file through the selected week. No ATS/cover column — this app doesn't compute one anywhere by design (see WeeklyBreakdownTab.tsx). No pipeline/data changes; all client-side from already-loaded `reg`/`teamWeekBySeason`. Verified: `npm run build` and `tsc -b --noEmit` green (after `npm ci` — the sandbox's global `tsc` was an unpinned v6.0.2 that spuriously failed on this repo's `baseUrl` option; the repo's pinned 5.5.4 has no such error), and via Playwright screenshots at desktop (1280px) and mobile (390px) viewports confirming the reworded table, working tooltips, and the modal opening with correct per-season pairs (spot-checked CAR 2015-2025) and 2025 game log.
- ✅ /data/grading_model (Season, Teams, Weekly, Features tabs) — contributions via contrib_params.json (weekContributions in lib/logic/contributions.ts). Weekly tab KPIs/rank/Z/percentile and Teams-tab avg scaled contributions (DAL 2025) verified exact vs pandas replica; season averages match. 2026-09-11: fixed a real bug (not scoped to mobile) where every axis-`name` label on this page's ECharts charts was rendered fully outside the canvas and invisible — confirmed by scanning rendered canvas pixels (0 dark pixels in the reserved margin) on the live site before the fix. Root cause: `grid.containLabel: true` reserves room for tick labels but not the axis `name` text, so `grid.bottom`/`nameGap` pairs left no space for the name to actually draw. Fixed by giving `grid.bottom` enough headroom (`nameGap + ~16-18px`) in `charts.tsx`'s shared `offDefScatterOption` (both desktop and the <520px mobile media query), `TeamsTab.tsx`'s two "Week" trend charts, and `WeeklyTab.tsx`'s grade histogram — the only 4 charts on this page with an axis `name`; `FeaturesTab.tsx`'s charts have no axis name and were unaffected. Verified via canvas pixel-margin checks (0px → 30-37px clear margin post-fix) on 3 of the 4 charts (couldn't force the 4th, TeamsTab's per-stat trend chart, to mount test data in the quick pass, but it's the identical one-line fix). `tsc --noEmit`/`npm run build`/62-test suite all green. Same-day follow-up: the Y-axis names (`Defense`, `Grade`, `Teams`, per-stat title) were also getting clipped where they sat horizontally above the axis — rotated all 4 to run vertically alongside the axis (`nameLocation: "middle"`, `nameRotate: 90`, `nameGap: 30`) and gave `grid.left` matching headroom (10 → 44, or 4 → 34 in the mobile media query); verified via the same canvas-pixel method (leftmost content column moved from touching/near 0 to a clean ~21-28px in from the edge).
...
- ✅ /game_analysis/matchup_previews (Week Preview, Matchup, Model Overview tabs) — shared engine in pages/game-analysis/previews/engine.ts (hist market rate w/ per-week exclusion via count subtraction, grades index, trend features). CAR@TB 2025 wk18 bundle verified exact vs Python replica (blend 45%, trend 46%, ML 42%, avg 44%). Model Overview computes all games client-side (~2s) instead of the planned model_overview.json export. 2026-09-15: mobile audit of the Matchup tab (Playwright screenshots at 375px, `npm run dev` + local data) — everything else already lays out cleanly (flex-wrap verdict strip, `overflow-x-auto` tables, stacked headers/snapshot), but the Trend Edge card's ECharts bar chart had 3 real clipping bugs at that width: its `h-40` container was too short for its own configured axis titles, so the y-axis name ("Edge contribution") overlapped the card header, the x-axis name ("Components (Δ away − home, weighted)") never rendered at all, and the 5th category label ("Last6 TO margin Δ") overhung the canvas's right edge and got cut. Fixed by dropping both redundant axis titles (the card's own description line + tooltip already explain the axes) and shortening the on-chart category labels via `axisLabel.formatter` (full names kept for the tooltip). Verified via screenshots on an empty (week 1, all-zero) and a real (DEN 2025 wk10, non-zero bars) matchup — all 5 labels render fully, no page-level horizontal scroll (`scrollWidth === clientWidth` at 375px). `tsc -b --noEmit` green. Same session, follow-up: Trend Edge had no "no data yet" fallback at all — before either team's first game it rendered a bar chart of all-zero bars (misleadingly implying a computed "no edge" rather than "not computed"), unlike Pythagorean's `MarginBars` which already showed "Not enough games yet" via a `slots.length < 2` check. `edgeComposite()` (lib/logic/edgeComposite.ts) already exposes this via `pAway: null` when either team has zero trend data (`noData()` helper) — reused that signal in MatchupTab.tsx to swap the chart for a "Not enough recent-form data yet" message, matching the Elo/Pythagorean cards' existing fallback style. Verified via screenshot: week 1 2026 (NE@SEA, both 0-0) shows the fallback text on both cards; a real week (DEN 2025 wk10) still renders bars normally. Not yet committed/pushed at time of writing this entry. 2026-09-25: Matchup tab "Recent form — last 3 games" block redesigned (`RecentFormTeam` in MatchupTab.tsx): teams now stacked (not side-by-side) so each gets a wider table; per team a W/L record chip + form dots, a 4-KPI strip (Pts/gm with allowed, Yds/gm with allowed, avg margin, total TO ±), and a per-game table ordered most-recent-first with grouped headers Game (Wk/Opp+logo/Result+margin) · Points (For/Agst) · Yards (For/Agst) · Diff (TO margin/EPA diff); the side that won each For/Agst pair is bolded. Ties now show "T" (previously fell through to "L"). Uses existing team_week columns only (points_allowed, total_yards_allowed, turnover_margin, epa_diff) — no pipeline changes. Verified via Playwright screenshots of ATL@GB 2026 wk3 at 1280px and 390px (no page horizontal scroll); `tsc -b --noEmit` green.
...
- ✅ `.github/workflows/weekly-refresh.yml` (cron Tue/Fri/Mon 12:00 UTC + dispatch → pipeline → validate → auto-commit → explicit `gh workflow run deploy.yml` — GITHUB_TOKEN commits don't fire push triggers). 2026-09-11: added Fri 12:00 UTC (after Thursday Night Football) and Mon 12:00 UTC (after Sunday Night Football) alongside the original Tue slot (after Monday Night Football) — same full `--stage all --refresh` pipeline runs on all three, grades/rankings just reflect whichever games have finished so far mid-week. Predictive-model refresh intentionally untouched (separate workflow/cron, not triggered by this file).
...
- `WinTypeDetailModal.tsx`: widened `x` prop from `number` to `number | string` and adjusted the title template (`${xLabel}${x !== "" ? ` ${x}` : ""} — win-type breakdown`) so a caller with no meaningful numeric `x` (both new callers pass `x=""`) doesn't render an awkward trailing value — existing Win Types usage (always a real number) is unchanged.
- `ThisWeekView.tsx`: both box plots now retain each box's underlying `Row[]` (`bySeasonRows` map for the season box plot; `similarWeeks[i].rows` for the comparison chart, alongside the `raws` used for the similarity stats) instead of only numeric spread values, and wire `chart.on("click", ...)` via `useECharts`'s `onInit` (same pattern as `WinTypesTab.tsx`'s `Block` component) into two refs (`seasonBoxEntriesRef`, `similarBoxEntriesRef`) rebuilt every render — refs rather than plain closures because `onInit` only fires once at mount, while the underlying rows change every time `selectedWeek` does. A click maps `dataIndex` → the ref entry → `classify()`d games → one shared `gamesModal` state → the modal. Added matching "Click a box to see…" hint lines under each chart's heading (same copy pattern as Win Types' bar-chart hint) and `cursor-pointer` on both chart containers.
- Verified: `npm run build` green. Dev-server + Playwright: clicking a "Spread by season" box opened "Season 2017 — win-type breakdown" (16 games, matches that season's Week-2 games); clicking a "Similar weeks" comparison box opened "Season 2020, Week 10 — win-type breakdown" (14 games) — both showed the full KPI/spread-strip/category-table popup unchanged from Win Types. Not committed/pushed yet at time of writing this entry.

### 2026-09-19 (cont. x12) — "This Week": revamp the spread-distribution section (histogram tooltip, box-plot fix, similar-weeks chart)
User asked for three things in Section 1 ("Spread distribution over the years"): a clearer histogram tooltip with the legend dropped, a fixed/clarified season box plot tooltip, and a new comparative box plot to the right of it, all three charts in one row. Found and fixed a real bug along the way, not just a labeling issue.
- **Histogram:** dropped `legend` (Mean/Median marklines already self-label on the chart); replaced the bare `tooltip: {trigger:"axis"}` with a formatter reading the hovered bar's `[lo, hi, count]` value tuple → "Spread -5.0 to 0.0 / 62 games".
- **Season box plot — root cause found:** the season/tooltip formatter (both the original and my first pass at "fixing" it) read `param.data` as a plain `[min, Q1, median, Q3, max]` 5-tuple. ECharts silently prepends the category index as an extra leading dimension for a category-axis boxplot series, so every field was off by one and the true max was never read at all — that's what the user saw as "Max/Min contradicting," not an abs-vs-raw sign issue. Fixed by having the tooltip formatter index into the *source* stats array via `param.dataIndex` instead of trusting `param.data`/`param.value` shape at all (same fix applied to the new similar-weeks chart, which would've had the identical bug). Verified by independently recomputing a season's quintile stats from `schedule.json` in Node and matching the tooltip exactly.
- Reordered the tooltip lines Max→Min (top-to-bottom, matching the box's visual top-to-bottom) instead of the old Min-first order.
- Added a `spreadMode: "abs" | "raw"` pill toggle (same visual pattern as the `sortMode` pills) shared by the season box plot and the new similar-weeks chart; abs/raw mode also swaps the Min/Max labels correctly (spread is home-perspective: negative = home favored, so in raw mode min = biggest home favorite, max = biggest road favorite).
- **New "Similar weeks (auto-detected)" chart:** per user's direction, no manual picker — every `(season, week)` REG combo with ≥4 games is a candidate, scored by normalized-Euclidean distance across `[mean, median, IQR]` of `|spread|` vs. the selected week's aggregate; top 3 shown, extended to 4–5 if within 25% of the 3rd-closest distance. Boxes shaded green→red by similarity rank (new local `lerpColor` helper; current week stays neutral brand blue).
- Section grid changed `lg:grid-cols-2` → `lg:grid-cols-3`.
- Verified: `npm run build` green; dev-server + Playwright screenshots on Week 2 and Week 10 — histogram tooltip/no-legend, season box plot tooltip values cross-checked against an independent Node recomputation of `schedule.json` in both modes (monotonic min≤Q1≤median≤Q3≤max, matches exactly), similar-weeks chart populates 5 comparisons with visible green→red shading and updates sensibly across different selected weeks. Not committed/pushed yet at time of writing this entry.
...
- `buildProbSpreadChartOption` now builds up to 3 scatter series per visible model — one per status a model actually has data in — using the app's existing `darkenColor`/`lightenColor` helpers (`engine.ts`, previously only used for the model-picker pills' selected/unselected states): correct = `darkenColor(..., 0.35)` at opacity 1, wrong = `lightenColor(..., 0.45)` at opacity 0.85, unplayed = the model's plain color at the original count-scaled baseline opacity. Draw order is still load-bearing (same lesson as cont. x9): all `unplayed` series first, then `wrong`, then `correct` last, so a correct pick from any model always wins the pixel on a shared bin rather than needing a separate overlay layer.
- Caption rewritten to describe the new encoding (dot size = games in that bin, color/opacity = outcome) instead of the retired "green outline."
- Verified: `npm run lint`/62-test suite green; dev-server check with all 12 seasons and all 7 models toggled on showed a clearly legible three-way split (dark/solid vs. pastel vs. normal-color-faded) with no ambiguous dots, even in the densest overlap clusters; 375px mobile screenshot confirmed no overflow and clean pill wrapping.

### 2026-09-19 (cont. x9) — "This Week": fix scatter green-outline paint order
User reported (with a screenshot) that a large multi-model overlap dot showed an outline that wasn't green while a smaller neighboring dot correctly was, and asked why. Root cause: the green "correct pick" outline was a per-item `itemStyle.borderColor`/`borderWidth` callback on each model's own scatter *series*, and ECharts paints series strictly in array order — whichever model happened to be later in `MODEL_KEYS` and landed on the same/adjacent binned coordinate painted its entire dot, border included, straight over an earlier model's dot. So a model that *did* pick correctly could have its green ring silently erased by a subsequently-drawn model's dot that didn't, purely due to list order — not a logic bug in `anyCorrect` itself, which was already correct.
- Fix: split the ring out of the fill series entirely into its own overlay series per model (`outlineSeries` in `buildProbSpreadChartOption`), containing only that model's `anyCorrect` bins, `silent: true` + `tooltip: { show: false }` (so it never intercepts hover — the real fill dot underneath still owns the tooltip) and `z: 10` so it always paints above every fill series regardless of array order. Fill series' `itemStyle` no longer sets a border at all (color/opacity only); the ring is always the true top layer now.
- Verified: `npm run lint`/62-test suite green; dev-server check with all 12 seasons and all 7 models toggled on (the densest possible overlap, worse than the reported screenshot) showed every "at least one correct pick" cluster with one clean, unbroken green ring, no cases of a ring missing or wrong-colored under an overlapping dot.

### 2026-09-19 (cont. x8) — "This Week": scatter hover lists games, outline now means correct not just played
Two follow-ups on the probability-vs-spread scatter: (1) hover only showed a bin's game count — now lists every underlying game (`{season} {away} @ {home} — {score}, {winner} won ✓/✗`, or "not yet played"), capped at 8 lines with a "+N more" trailer for heavily-binned spots. (2) The green outline was keyed off `anyPlayed` (any game in the bin graded, regardless of whether the model was right) — the user correctly called this out as misleading. Renamed to `anyCorrect`: a bin now derives the model's predicted side via `pickWinner` (re-imported from `engine.ts`, reused rather than reimplementing the tie convention) and only outlines green if at least one underlying game had that model's pick match the actual winner.
- `ProbSpreadPoint` gained a `games: ProbSpreadGame[]` list (season, teams, scores, winner, per-game `correct`); `buildProbSpreadPoints`/`buildProbSpreadChartOption` updated accordingly; caption text under the chart heading rewritten to describe hover + the corrected outline meaning.
- Verified: `npm run lint`/62-test suite green; dev-server check hovered a real multi-season bin and confirmed the tooltip's per-game list, scores, and ✓/✗ marks were individually correct (e.g. "2023 BAL @ CIN — 27-24, BAL won ✗" for a Market-calibrated-favors-home dot where the away team actually won), and that the dot's green outline correctly tracked the presence of a ✓ line rather than just "played"; 375px mobile check showed no overflow.

### 2026-09-19 (cont. x7) — "This Week": probability-vs-spread scatter under the disagreement table
New chart under "Does model disagreement predict upsets?": Y-axis is the signed spread (home perspective, matches Section 1's histogram), X-axis is each model's own P(home wins), one scatter series per model. Confirmed the design with the user first (AskUserQuestion): signed spread axis, a custom pill row for model toggling (not the tiny built-in ECharts legend), and the season selector starting on just the currently-paginated season with chips to add more.
- Also fixed a real UX confusion flagged by the user while scoping this: the existing wide-spread row border/tag was green, which read as "this team won" even though it never meant that — recolored to violet (`DotStripRow`, both the border and the "Spread N.N" tag), freeing green to mean exactly one thing on this page now: "this game's result is known," used by the new chart's dot outlines.
- New `buildProbSpreadPoints` bins each model's (P(home wins), spread) pairs to a coarse grid (probability to the nearest 4pp, spread to the nearest 1pt) so near-identical games collapse into one marker instead of overplotting — each bin tracks `count` (drives marker size + opacity, capped) and `anyPlayed` (drives a green outline when at least one game in that bin is graded; a deliberate bin-level simplification, documented inline). `buildProbSpreadChartOption` renders one series per bin set — toggled-off models are simply omitted from the series array rather than hidden via legend state, so the new model pill row (`visibleModels` state) *is* the legend, no ECharts legend/selectedMode syncing needed. New `selectedGraphSeasons` state (independent of the card-list season pager) seeds once to just that pager's current season; season chips add/remove seasons from the plotted set live.
- Verified: `npm run lint`/62-test suite green; dev-server check confirmed the wide-spread border/tag is now violet (not green) and visually distinct from the new chart's green "known result" outline; toggling a model pill removed exactly that series; adding older seasons (2023-2025) visibly densified the scatter with clearly larger, green-outlined markers where games overlap; 375px mobile check showed both pill rows wrapping cleanly with zero horizontal overflow.

### 2026-09-19 (cont. x6) — "This Week": visible spread tag on every row
The wide-spread border highlight had no visible number attached — the reason was only in a hover-only `title`. Added a small pill "tag" notched directly onto each row's top border (the `<fieldset><legend>` visual pattern), shown on *every* row (not just the highlighted ones) so the actual spread is comparable across the whole slate: `Spread {n.n}`, colored to match the border it sits on — green when it's the wide-spread state (`|spread_line| > 6`), neutral gray otherwise. Bumped the row list's gap from `space-y-2` to `space-y-4` so the tag has room to sit above each row without crowding the one above it.
- Verified: `npm run lint`/62-test suite green; dev-server check confirmed every row shows its tag, the one wide-spread row in the current slate (CLE@TB, 8.5) got the green variant matching its green border, and all others show the gray default; 375px mobile check showed the tag intact and unclipped with zero horizontal overflow.

### 2026-09-19 (cont. x5) — "This Week": badges show numbers directly, wide-spread border highlight
Two follow-ups on the listening-recommendation badges: (1) the win% and sample size were only in the hover title, invisible on touch — now every badge shows two lines directly on the card: the model/category name, and `{win%} (n={sample size})`, for all three tones (previously only "all agree" showed a number at all). Long model labels get a `SHORT_LABEL` map for the badge itself (full name still in the title) so the widened badge (`w-24 sm:w-28`) doesn't overflow. (2) `DotStripRow`'s card border now turns green and doubles in thickness (`border-2 border-emerald-500` vs the default `border border-slate-200`) when `|spread_line| > 6` — a quick visual flag for wide/blowout-line games while scanning a season's slate.
- Verified: `npm run lint`/62-test suite green; dev-server check confirmed a real wide-spread game (CLE@TB) got the thicker green border while others didn't, and all three badge tones now show their percentage/n directly; 375px mobile check showed rows still fitting on one line with both badge lines legible and zero horizontal overflow.

### 2026-09-19 (cont. x4) — "This Week": per-row "listening recommendation" badges
The agreement matrix (kept, per "keep all changes") wasn't the actionable read the user actually wanted. They asked for three specific conditional-accuracy questions, applied per game row: when all models agree, what's the real win rate of that side; when two specific models disagree, which one has historically been right more often in exactly that split; when models cluster near 50%, which model is actually best in toss-ups. Wanted as annotations to the right of each dot-strip row.
- `previews/modelAgreement.ts`: exported `sideOf`; added `computeAllAgreeStat` (win rate of the agreed side, among games where every non-consensus model picks the same side), `computePairwiseResolution` (for every pair among the 6 non-consensus models, each one's own accuracy restricted to games where that pair disagrees, plus which is `better`), `computeTossUpAccuracy`/`bestTossUpModel` (per-model accuracy restricted to that model's own 45–55% calls), and `categorizeGame` (classifies one game as toss-up / all-agree / disagree-between-two-named-models, toss-up taking priority since it's decision-relevant regardless of agreement).
- `ThisWeekView.tsx`: three new memos computing the aggregates from `gameModelRows` (same full-multi-season scope as the KPI row and matrix, so the underlying win-rate/accuracy numbers stay stable while paging seasons — only which category a given row falls into changes). New `buildAnnotation()` turns a game's category + the aggregates into display-ready `{tone, text, title}`; a small `RowAnnotation` badge (🤝 emerald / 🪙 amber / ⚡ sky, native-title tooltip with the full stat) renders at the end of each `DotStripRow`, computed once per row in the `cardsForSeason` memo rather than per-render. A one-line legend above the row list explains the three badge types. Low-sample badges (n<5) render at reduced opacity rather than asserting a confident recommendation off a handful of games.
- Verified: `npm run lint`/62-test suite green; dev-server check spot-verified badge/dot-position consistency across several rows (an "all agree" row genuinely had every dot on the same side of 50%, a "toss-up" row's dots straddled 50% evenly); 375px mobile check showed every row staying on one line with the badge intact and zero horizontal overflow.

### 2026-09-19 (cont. x3) — "This Week": pairwise model-agreement matrix
Added relationship analytics between the 7 models to Section 2, placed to the right of the KPI/avg-confidence-strip row. Presented the user 4 concrete options (ranked scoreboard, pairwise agreement matrix, accuracy-vs-confidence quadrant, combo) — user picked the pairwise agreement matrix.
- New `previews/modelAgreement.ts`: `AXIS_KEYS`/`AXIS_LABELS` (the 7 `MODEL_KEYS` plus an 8th "Actual" entry) and `computeAgreementMatrix(rows)` — for every axis pair, the % of this week's games (across all seasons, same scope as the KPI row above it) where both sides' picks agree, reusing `pickWinner` from `engine.ts` (no new pick-derivation logic). Treating the real winner as an 8th axis entry means the (Model, Actual) cell is just that model's accuracy — so one 8×8 matrix answers both "how do the models compare to each other" and "how does each compare to the real winner," with no second component needed.
- Rendered as an ECharts heatmap in `ThisWeekView.tsx` (`buildAgreementChartOption`), modeled on `previews/ModelPickerTab.tsx`'s existing model×week/season heatmap shape.
- **Bug found and fixed along the way, in new code (not shipped elsewhere by this session) plus a pre-existing one spotted in already-shipped code**: ECharts' heatmap series requires a `visualMap` component (throws "Heatmap must use with visualMap" otherwise — first attempt to drop it crashed the whole page), and an `itemStyle.color` callback does **not** override that visualMap's coloring the way `ModelPickerTab.tsx`'s existing heatmap assumes — confirmed live that `ModelPickerTab.tsx`'s own heatmap has the identical latent bug (its "no data" cells and its 100%-accuracy cells both render the same washed-out yellow instead of gray/green — not fixed here, out of scope, flagged for a future pass). Fixed for this new matrix by routing color entirely through the visualMap's `inRange` (continuous red→amber→green, amber landing at the 50% "coin flip" midpoint) instead of a callback, and using a `null` third value (ECharts' own "blank cell" convention) for the diagonal rather than an out-of-range sentinel (a `-1` sentinel below `min` was still clamped into the in-range red end rather than triggering `outOfRange`, confirmed live).
- Verified: `npm run lint`/62-test suite green; dev-server check confirmed the "Actual" row/column's values (65/62/61/59/61/53/54) match the existing model-accuracy pill row exactly; 375px mobile check showed the matrix wrapping cleanly below the KPIs/confidence strip with zero horizontal overflow.

### 2026-09-19 (cont. x2) — "This Week": simplify Section 2's game rows to dot-strip + logos only
Follow-up: the v2 card grid (logos, probability bar, pick badge, dot strip) was still more than the user wanted — asked for "ONLY the bars that contain the model dots, and on each side the logos" so games are more easily comparable side by side. Replaced `GameCard` with a much simpler `DotStripRow`: just the two team logos (winner still gets the colored ring) flanking a full-width `ModelDotStrip`, no date, no probability bar, no pick badge. Changed the layout from a multi-column card grid to a vertical list of full-width rows so every row's track spans the same width — dot positions now line up vertically from one game to the next, which is what actually makes them "easily comparable" (a grid of narrower cards couldn't do this). `ModelDotStrip` (`previews/ModelDotStrip.tsx`) gained an optional `showEndLabels` prop (default `true`, so `WeekPreviewTab.tsx`'s existing usage is unaffected) to suppress its own tiny `← AWAY`/`HOME →` text labels when a caller already shows real logos beside it. Season pager, sort modes, and the per-model accuracy/primary-picker row above the list are unchanged.
- Verified: `npm run lint`/62-test suite green; dev-server check confirmed rows render with aligned tracks across a season's slate and no regression on Matchup Previews; 375px mobile check showed zero horizontal overflow with rows scaling down cleanly.

### 2026-09-19 (cont.) — "This Week" v2: fix live-week bug, rebuild Section 2 as card grid, polish
Follow-up after live user testing of the "This Week" tab surfaced four issues.
- **Bug fix — ML Fair/Elo showing "—" for the live/upcoming week**: root cause was the live-week code path feeding raw `predictive_model/upcoming.json` rows straight into `probBundle()`, but that file has no `game_id`/`home_moneyline`/`away_moneyline` (only `season, week, home_team, away_team, gameday, predicted_margin, home_win_prob, home_covers_prob, spread_line, market_home_fair, elo_p_home`) — so the Elo index lookup (`eloIdx.get(game.game_id)`) and ML-fair's moneyline read both silently missed. Fixed by dropping that separate code path entirely: `schedule.json`'s own row for the live/future week already carries `game_id`/odds/moneylines (confirmed live), and it's already included in `weekSchedRows` (schedule rows for the selected week number span every season, played or not) — so the live week is now just the normal historical-game pipeline, filtered to the most recent season, with no special-casing and no bug surface. Verified live: NYG@LA (2026 wk2, unplayed) now shows real ML Fair (24/76%) and Elo (26/74%) values, both previously "—".
- **Section 2 replaced entirely** with a paginated grid of per-game cards, per a screenshot the user provided of a pattern that turned out to already exist in this app: `ModelDotStrip` (pill track, one colored dot per model's home-win %, consensus as a bar marker, 50% tick, `← AWAY`/`HOME →` end labels, tap/hover popover) was previously defined only inline in `previews/WeekPreviewTab.tsx` — extracted to a new shared `previews/ModelDotStrip.tsx` (`ModelDotStrip`, `disagreementOf`) so both pages use one implementation; `WeekPreviewTab.tsx` now imports it instead of duplicating it (verified byte-identical render on Matchup Previews after the extraction). New `GameCard` (bespoke to `ThisWeekView.tsx`) mirrors `WeekPreviewTab.tsx`'s card — two team logos, a color-split probability bar for the selected primary model, a pick badge, `ModelDotStrip` below — plus, per user request, a winner ring (colored box-shadow) + bold score on the actual winning team's logo, independent of any model's pick-correctness checkmark. Cards are paginated by season (newest first — "starting with last season"), with a `Select` + prev/next stepper (same `stepBtnCls` pattern as `GamePicks.tsx`); sortable by Time/Highest prob/Disagreement (reused verbatim from `WeekPreviewTab.tsx`'s existing three-way toggle); a clickable per-model accuracy pill row (also reused from `WeekPreviewTab.tsx`) doubles as the primary-model picker. New `AvgConfidenceStrip` (bespoke, not `ModelDotStrip` reuse — different domain) shows each model's average *confidence* (probability assigned to whichever side it picked, always ≥50%) across all seasons' games for the week, on a "Toss-up → Lock" track — reframed from raw home-win-% since that isn't meaningful pooled across different matchups. Dropped: the old 0–100% line chart and its horizontal-scroll wrapper (`colChartW` removed from `components/charts/sizing.ts`, zero other references), and the flat "every historical game" table (superseded by the card grid). Kept: the disagreement-bucket "does disagreement predict upsets" table.
- **Section 1 histogram polish**: local-only `prettyHistogramSeries()` in `ThisWeekView.tsx` (gradient fill + rounded top corners via a custom `renderItem`, same mechanism as the shared `histogramBarSeries()` but not touching that shared helper, since `WeeklyTab.tsx`/`ConfidenceTab.tsx` depend on its current look) plus restyled mean/median markLines (solid navy / dashed orange). Box plot unchanged.
- **Section 3 additions**: kept the existing r-by-lag analysis, restyled the bar chart (gradient fill, rounded corners, still plain `itemStyle`/`markLine`, no shared-helper changes needed since bar series supports this natively). Added a new per-team momentum table ("which teams carry momentum from Week N-1") — win rate in the selected week after a Week N-1 win vs. after a loss, per team with logo, sorted by swing magnitude — answering the "missing per-team detail" feedback without changing the underlying pooled analysis.
- Verified: `npm run lint` (tsc -b) clean, 62/62 Vitest tests green, dev-server smoke test with real data (2026 wk2) confirmed the bug fix via clicking individual model dots' popovers, and a 375px mobile check showed zero horizontal overflow across the new card grid, KPI/confidence-strip row, and momentum table.

### 2026-09-19 — Pick'em Recommendations: new "This Week" tab
New third sub-view on Spread Analytics → Pick'em Recommendations (alongside Recommendations/The Story), for `#/game_analysis/spread_win_percentage/pickem`. Single filter is a REG-season **week number**, aggregated across every season in the dataset (not a season+week pair) — the goal is "given history for this week number, how will the current instance of it play out."
- New `pickem/ThisWeekView.tsx` + `lib/logic/weekHistory.ts` (`pearsonCorrelation`/`correlationRead` — the only new math; every model probability is reused wholesale from `previews/engine.ts`'s `probBundle`/`buildHist`/`buildGradesIndex`/`buildTeamWeekIndex`/`buildScheduleEloIndex`/`buildPredictiveIndex`, not reimplemented). `PickemRecommendationsTab.tsx` widened to a 3-way `Segmented`.
- **Section 1 — spread distribution over the years**: KPI row (games, seasons covered, avg/median/std/IQR/min-max of |spread_line|, `contributions.ts`'s `percentile`/`sampleStd`) + histogram (signed spread, `histogram.ts`) + a per-season box plot (min/q1/median/q3/max of |spread| per season) showing how the week's spread has moved year to year.
- **Section 2 — all models' behavior**: for every historical game in the selected week (across seasons), runs the full `probBundle()` (production regression, market/ML-fair, market-calibrated blend, Elo, Pythagorean, Trend Edge, consensus — `MODEL_KEYS`/`MODEL_COLORS` reused so colors match Matchup Previews) — a model is simply absent when it doesn't apply yet (e.g. Pythagorean/Trend Edge at week 1, no special-casing needed, `probBundle` already encodes it). A 0–100% line chart plots every model's home-win-probability per game plus the actual winner; below it, a disagreement-bucket table (max−min of available model probs) checks whether model disagreement correlates with the underdog winning, and a full per-game table lists every historical game sorted by disagreement with max/min cells highlighted.
- **Section 3 — correlation vs every prior week**: per user's explicit ask, not just week-1-vs-selected-week — computes a per-team win/loss Pearson r between the selected week and *each* prior week (1..selected-1) from `team_week/{season}.json`'s `win` column, charted by lag so the reader can see whether the immediately preceding week correlates more strongly than distant ones (it is not assumed to — the chart just shows what the data says). A concrete conditional win-rate breakdown ("win rate in Wk N after Wk N-1 win/loss vs. baseline") illustrates the strongest/most-recent case. Week 1 shows an explanatory empty state (no prior weeks).
- A live "this week's games" banner (using `predictive_model/upcoming.json` run through the same `probBundle` treatment) surfaces above the sections when the selected week equals the real upcoming week.
- Verified: `npm run lint` (tsc -b) clean, 62/62 Vitest tests green, `npm install` + dev server smoke test — screenshotted Week 2 (current live week, banner renders) and Week 1 (correctly shows `—` for Trend Edge/Pythagorean and the Section 3 empty state) with real data, all three sections render sensibly, zero new console errors (one pre-existing `TabBar.tsx` React-key-spread warning, unrelated).
...
- `engine.ts`: new `alignedMarginTimeline(twIdx, awayTeam, homeTeam, season, wkPlayed)` — simpler than `alignedEloTimeline` since Pythagorean's inputs (Σpoints/Σpoints_allowed) are season-scoped with no cross-season carryover to align across; just unions both teams' played weeks (from the same `twIdx.rowsFor` already powering `keyStats.pythAway/Home`) through `wkPlayed`, sorted. A week only one team played is `null` for the other side.
- `MatchupTab.tsx`: new `MarginBars` component — ECharts grouped bar chart, one series per team colored by `meta.get(team)?.color`, zero baseline included by default (no `scale:true` needed unlike Elo's off-zero line), `weekLabel()` maps weeks 19-22 to WC/DIV/CON/SB for postseason. Axis-trigger tooltip lists whichever team(s) played that week (`{team} {scored}-{allowed} ({margin}) vs {opponent} · Win/Loss`) — no dual-line nearest-cursor trick needed since bars are already spatially unambiguous.
- Pythagorean `ModelBlock` rewired: KPI row (team-dot + abbreviation + expected win%, same convention as the Elo card) → `MarginBars` → one footnote combining the existing points-scored/allowed explanation with the log5 methodology note. The three old `ProbBar`s (away/home expected win%, log5) are gone.
- **Verified**: CLE @ BAL 2024 Wk18 — bars render with correct team colors and values, CLE's and BAL's *different* bye weeks each show as a real gap (not backfilled), hover shows exact scored-allowed/margin/opponent/result for both teams at once. Week-1 fallback ("Not enough games yet") renders cleanly. **Noted finding**: the postseason/`weekLabel` branch is currently unreachable in practice — the Matchup tab's season/week selectors are built only from `game_type === "REG"` games, so a playoff game can never actually be the *previewed* matchup, and `wkPlayed` therefore never exceeds 17. The code is harmless and correctly written for if that ever changes, but couldn't be exercised through the UI as planned.
- `npm run build` / `tsc --noEmit` clean.

### 2026-09-14 (cont. x14) — Elo sparkline: fix a real bug the "connectNulls:true" fix missed
User feedback caught a real flaw in the previous fix: once the previewed matchup is far enough into the new season that a non-playoff team has real games on *both* sides of the other team's entire postseason run, blanket `connectNulls: true` would bridge straight across all of it (multiple weeks, spanning a season boundary) — not just an ordinary bye. Confirmed and reproduced with KC (2024 AFC's #1 seed, bye + DIV/CON/SB) @ JAX (missed the playoffs) at 2025 Wk5: JAX's window now has real 2025 games after KC's entire postseason gap, so the old logic would have wrongly connected 2024 straight into 2025.
- Replaced the blanket flag with explicit per-slot detection in `EloSpark` (`eloSeriesData`): a `null` gets filled with an **interpolated value** (average of its immediate neighbors, rendered with `symbol: "none"` — no dot) only when both neighbors are real **and in the same season** — the precise definition of an ordinary bye. Any other null (a multi-week postseason run, or a single gap whose neighbors fall in different seasons — confirmed with Detroit's real 2024 case: a #1 seed with just one playoff loss still creates a season-crossing single-slot gap for its opponent, correctly left as a break) stays `null`, and `connectNulls` is now `false` on both series since the real byes are already filled in as actual data points.
- Re-verified KC @ JAX 2025 Wk5: JAX's line now shows a single interpolated/dot-less bye mid-window, and a genuine visible break spanning KC's 3 playoff games, confirmed both in the raw series data and visually (screenshot). Re-checked DET (2024 #1 seed, one playoff loss) @ CIN (missed playoffs) as a second case: CIN's single missing slot correctly stays a real break (its neighbors span the 2024/2025 season boundary) rather than being treated as an ordinary bye.
- `npm run build` / `tsc --noEmit` clean.
...
- `MatchupTab.tsx`: `EloSpark` now takes one `slots: EloTimelineSlot[]` prop instead of separate `awayPts`/`homePts` (dropped `padElo`, which only ever padded at the *start* for a shorter overall history — the real, mid-timeline gap case wasn't handled at all before). **`connectNulls` flipped from `true` to `false`** on both series — this is the actual visual fix requested: a null is now a real break in the line, not bridged across.
- Verified in-browser with DAL @ PHI 2025 Wk1 (PHI won the 2024 Super Bowl, DAL missed the playoffs): DAL's line visibly stops after its last regular-season game while PHI's continues through 4 more playoff weeks to the right edge; confirmed at the data level too (DAL `null` at the last 4 indices, plus a `null` mid-timeline for a bye week; PHI has real values across all 17). Hovering a PHI-only index (a real Divisional-round game) shows only PHI's info, no crash, nothing fabricated for DAL.
- `npm run build` / `tsc --noEmit` clean.

### 2026-09-14 (cont. x11) — Predictive pill: per-game "biggest movers," team-colored
User feedback: the top-5 list's % was global permutation importance — the same fixed list/percentages on every single matchup, no real per-game meaning. Asked to focus on the actual stat and its impact instead; then (mid-fix) to also color by which team's own color the driver favors, not a generic green/red.
- Rebuilt `topPredictiveDrivers` (engine.ts) around this game's own `_contrib` values instead of `importance.json`. To avoid the sign/magnitude instability this ran into last time raw coefficients were shown (docs/predictive-model-decision.md's collinearity warning — confirmed then with a real example), first **collapses known collinear families** (`FEATURE_FAMILIES`: Elo + its own sq/sqrt transform; start-field-pos and start-EP the same way; overall/offense/defense grade + overall's own sq/sqrt; total/pass/rush L3 EPA diff) by summing each family's `_contrib` — the redundant split is exactly what cancels when summed, leaving the concept's real net effect on the predicted margin. Ranks the resulting ≤~30 concepts by |summed contribution| **for this specific game** — now genuinely varies matchup to matchup (verified: NE@SEA Wk1, with no rolling history yet, surfaces small context movers like `elo`/`div_game`/`temp`; CLE@BAL Wk18 surfaces `elo +10.4`, `l3 epa diff −5.1`, `cum overall grade +3.8`, etc.).
- Dropped the `predImportance` prop/loading entirely from `MatchupPreviews.tsx`/`MatchupTab.tsx` (`topPredictiveDrivers` now only needs the one game's feature row — it reads the column list directly off that row's own `..._contrib` keys, no separate feature-list input).
- `ContribRow`: value is now a signed points figure ("+10.4"/"−5.1" points of the predicted margin) instead of a %; bar width is relative to the largest of the shown 5 (visual only). Color — bar and value — is the actual favored team's own brand color (`meta.get(team)?.color`), matching the Elo card's team-color convention, replacing the earlier flat green/red. Hover explains the feature (noting how many collinear inputs were combined), the real per-team values, and the point impact "net of every other factor in the model."
- Section header renamed "Top drivers of this prediction" → "Biggest movers for this game (margin points)".
- `npm run build` / `tsc --noEmit` clean.

### 2026-09-14 (cont. x10) — Fix: 2026 games missing Predictive model data
User reported the Predictive card showed blank/no-breakdown for 2026 games (while consensus still showed a %) — regression-shaped, but actually a pre-existing gap the new pill just made visible.
- **Root cause** (via git log + `gh run list` + direct file inspection): the *historical* export (`pipeline/predictive_model/export_page.py` → `games.json`/`game_features.json`/`importance.json`/`meta.json`) hadn't been regenerated since 2026-07-25 and had **never** contained a season-2026 row (`meta.json.test_seasons` topped out at 2025) — it's a manual-only script, not wired into any scheduled workflow, unlike everything else in this repo. The *live* export (`export_upcoming.py` → `upcoming.json` family) was healthy and cron-fresh but by design only ever covers the single "next unplayed" game. Since `predIdx`/`predFeaturesIdx` merge exactly those two sources, every 2026 game except the one live pick had zero coverage. The "overall %" the user saw was `bundle.consensus` (nanmean over 6 sub-models, silently drops the null predictive term) — not an alternate predictive source.
- **Leakage review before implementing** (user asked explicitly): confirmed `features.py`'s per-team rolling/expanding features (`_rolling_trend`, `_surprise_rolling`, `_cumulative_grades`) are all `groupby(team,season).shift(1)`-then-window, strictly backward-looking regardless of whether a season is complete or in-progress; the regression pipeline only `.fit()`s on strictly-earlier seasons. Also noted this exact "score an in-progress season's own completed games" scenario is already exercised weekly in production by `export_upcoming.py` without incident. No leakage risk from adding 2026 to the historical export's test-season range.
- **Fix**: added `export_page.py` as a step in `.github/workflows/predictive-refresh.yml` (before `export_upcoming.py`, same job/commit), and broadened the `git-auto-commit-action` `file_pattern` to include everything it writes (`games.json`, `game_features.json`, `season_summary.json`, `importance.json`, `calibration.json`, `meta.json`) alongside the existing `upcoming*.json` three.
- Ran both exports locally to fix production today rather than waiting for Tuesday's cron: `meta.json.test_seasons` now includes 2026 (15 completed games so far), confirmed via direct JSON inspection. Verified in-browser: NE @ SEA (an already-played 2026 Wk1 game) now shows a real `Predicted home win prob.` and a populated top-5 driver list; DEN @ KC (the live upcoming pick) still works exactly as before.
- `npm run build` clean — no frontend/engine.ts code changes needed, as expected (the existing merge/lookup just needed real data to find).

### 2026-09-14 (cont. x9) — Matchup Previews: Predictive pill reworked — top-5 drivers, real values
User asked to remove the pill's long note (referencing Models Guide instead), and show what's leading to the prediction: top 5 variables, % contribution, actual values, more detail on hover.
- **Pipeline**: per-game feature contributions already existed for every historical game (`game_features.json`, `export_page.py`) but not for the live upcoming week. Added the same decomposition to `export_upcoming.py` (reuses the model already fit for the prediction — no new fit, just `impute → scale → coef_` on data already in memory) writing a new `upcoming_features.json`. Ran it locally to regenerate the data. **Caught and fixed a real deploy-breaking gap**: `predictive-refresh.yml`'s `git-auto-commit-action` `file_pattern` only listed `upcoming.json`/`upcoming_meta.json` — without adding `upcoming_features.json` to it, the new file would compute correctly in CI every week but never actually get committed.
- **Data-quality finding during verification** (not a bug in this session's code — a property of the model): raw per-game linear coefficient contributions for individual features can be enormous and near-cancelling for this model's several collinear feature groups (Elo alongside its own square/sqrt transform; total EPA diff alongside its pass/rush split) — confirmed by hand (BAL's clearly-better raw L3 EPA diff for a real game still had that feature's own coefficient contribution nominally "favoring" the other team). This matches `docs/predictive-model-decision.md`'s explicit warning that raw coefficients are unstable here and permutation importance should be preferred. Redesigned around this: `topPredictiveDrivers` (engine.ts) picks the top 5 by **global permutation importance** (`importance.json` — stable) rather than by-game |contribution|, computes each one's **% share of the shown 5's combined importance** (always sane, sums to 100%, never negative), and pulls this game's **real per-team raw values** (reliable regardless of collinearity) from the new features index — but deliberately makes **no per-feature "favors this team" claim**, since that sign is exactly what's unstable.
- New `PredictiveFeaturesIndex` (engine.ts) merges `game_features.json` + `upcoming_features.json` the same way `predIdx` already merges `games.json` + `upcoming.json`; `predImportance` (from the existing `importance.json` loader) is fetched alongside it in `MatchupPreviews.tsx` and passed only to `MatchupTab`.
- Pill UI: reused `labelFor`/`describeFeature` (already existed in `pages/predictive-model/`, built for the standalone Predictive Model page's "Game inspector") instead of inventing new copy. Each row: label, a bar sized to importance share, the %, and a compact "AWAY x · HOME y" actual-value note; hover (native `title`, multi-line) adds the full feature description plus the diff. The long inline note is gone, replaced by one line ("Linear regression on pre-game stats predicts the scoring margin.") linking to `#/game_analysis/models_guide`.
- Verified end-to-end in-browser: a historical game (CLE @ BAL, 2024 Wk18) and the live next-week game (DEN @ KC, 2026 Wk1, from the brand-new `upcoming_features.json`) both render correct top-5 lists with sane percentages and real values (the live game correctly omits the actual-value note for rolling-window features that don't exist yet in Week 1).
- `npm run build` / `tsc --noEmit` clean.
...
- `position` callback: computes both lines' pixel Y via `convertToPixel` as before; if they're within 6px of each other (visually touching/overlapping), renders **both** teams' blocks separated by a thin divider instead of forcing a nearest-line pick a real cursor couldn't reliably make; otherwise picks the nearer one as before.
- Verified in-browser via `zr.handler.dispatch('mousemove', ...)` + reading the live tooltip DOM: hovering near CLE's exact pixel returned the correctly-ordered single-team block (`CLE — Elo 1496` / `Week 19 · 2023 · Loss vs HOU`); this matchup's closest same-index gap between the two lines was 16.7px (above the 6px merge threshold) so the merge branch itself wasn't exercised against real data this session — it's the same `convertToPixel`-based distance check already proven correct for the nearest-line pick, just compared against a threshold instead of against the cursor.
- **This is the change that actually reaches production** — the prior two sessions' hover fixes existed only in the working tree; committed and pushed together with this one.
- `npm run build` / `tsc --noEmit` clean.

### 2026-09-14 (cont. x5) — Matchup Previews: Elo sparkline hover — fixed regression, working forgiving hover
User reported the previous fix (invisible thick overlay lines) broke hover entirely. Root-caused and replaced with a different, more robust approach.
- Reverted to two plain line series (no invisible overlay lines) — `trigger:"item"` (needs the cursor within a couple px of the 2px line) is what "worked well" before, but isn't itself forgiving; the fix instead makes the whole axis-wide interaction forgiving by switching to `trigger:"axis"` (fires anywhere along the x-axis, not just within a few px of a specific line) plus custom logic that shows whichever of the two teams' points is closer to the cursor.
- First attempt tracked mouse Y via a separate `zr.on("mousemove", ...)` listener into a ref read inside `formatter` — this reproduced the reported "hover doesn't work" bug: the listener fires in registration order, so ECharts' own internal tooltip mousemove handler (registered earlier, during `setOption`) ran the formatter using the *previous* event's mouse Y, one tick behind ours. Confirmed via direct zrender `handler.dispatch('mousemove', {zrX,zrY})` + reading the tooltip DOM in-browser: switching Y between two known pixel positions reliably showed the wrong (stale) team.
- Fixed by moving the "which line is closer" decision into `tooltip.position` instead of `formatter` — `position(point, params, dom)` receives the *actual* mouse point synchronously in the same call, with no separate listener/race, so `dom.innerHTML` is overwritten there with the correct single-team content (formatter just returns `""` as a placeholder). Verified in-browser via repeated `zr.handler.dispatch('mousemove', ...)` at CLE's exact pixel, BAL's exact pixel, a midpoint nudged toward each, and back to CLE again — every case returned the correct team immediately, no lag.
- Net effect vs "before": hovering the general vertical strip near either line (not just exactly on the 2px stroke) now shows that line's info — meets the original "closer to the corresponding line" ask without the regression.
- `npm run build` / `tsc --noEmit` clean; win/loss dot and season-divider pixels still present (canvas sampling), same as prior sessions.
