...
- ☐ Optional: screenshot-based visual QA (browser pane screenshot capture currently times out on this app). **Now owned by M6** — the mobile audit (2026-07-29) confirmed this gap is the reason no layout problems were caught: M4's scope was visual *consistency* (verified by DOM audit), never *responsiveness*. The Session-2 note below promised a "responsive/UI polish pass" that was never added to this checklist. See `docs/MOBILE_READINESS.md`.
...
- ☐ Phase 2 chart size/axes/legends · ☐ Phase 3 bin correctness · ☐ Phase 4 touch & interaction · ☐ Phase 5 per-page layout · ☐ Phase 6 tables · ☐ Phase 7 mobile performance
- **Bin defects found (affect desktop too, not just mobile):** `WeeklyTab.tsx:96,107` places the histogram's mean/median `markLine`s by integer bin index on a *category* axis, so they render at the containing bin's **center** rather than the true value (off by up to half a bin width); `WeeklyTab.tsx:74` is over-binned (16 bins for 32 teams). Both fixed by converting the histogram to a value axis. Also: `lib/logic/spreadBins.ts` is dead code (zero importers) duplicating `SpreadWinPct.tsx:65-71` and carrying an always-empty-top-bin bug the live version doesn't have; two `percentile` functions use opposite unit conventions (0–100 vs 0–1); `shared.ts:194`'s "mirrors calibration_table exactly" comment overclaims (`[lo,hi)` vs the pipeline's `(lo,hi]`).
