...
export default function SpreadAnalytics() {
  ...
  return (
    <div className="space-y-4">
      ...
      <TabBar tabs={TABS.map(([t, , icon, desc]) => [t, icon, desc] as const)} active={tab} onChange={setTab} gridClassName="sm:grid-cols-2" />

      {tab === "Win Rate & Calibration" ? <WinRateTab /> : <WeeklyBreakdownTab />}
    </div>
  );
}
