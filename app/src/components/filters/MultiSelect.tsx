...
export function MultiSelect({ label, values, options, onChange }: MultiSelectProps) {
  ...
  return (
    <div ref={ref} className="relative flex flex-col gap-1 text-xs font-medium text-slate-600">
      {label}
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="min-w-36 rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm text-slate-900 shadow-sm focus:border-[#002f6c] focus:outline-none"
      >
